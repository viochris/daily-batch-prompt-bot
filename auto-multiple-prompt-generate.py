...
def generate_image_prompts():
    """
    Task to generate three (3) creative image generation prompts using Gemini.
    
    The output is a single raw string containing 3 prompts separated by double newlines,
    formatted for direct processing.
    """
    ...
    Your task is to generate EXACTLY THREE (3) creative image prompts.

    ### 🎨 STYLE GUIDELINES:
    1. **Structure:** A descriptive main subject followed by comma-separated artistic keywords (lighting, style, vibe).
    2. **Creativity:** Choose random themes (Cyberpunk, Fantasy, Horror, Realistic, or Abstract).
    3. **Format:** Output ONLY the raw text.
       - Do NOT use numbering (1., 2.).
       - Do NOT use quotes ("").
       - Do NOT use Markdown (**).
       - Separate each prompt with TWO NEW LINES (Double Enter).
    ...
    ### 🚀 YOUR TASK:
    Generate 3 new, unique image prompts now. Separate them with double new lines.
    """
    ...
    try:
        ...
        if response.text and len(response.text.strip()) > 0:
            ...
            print(f"✅ Generated Prompts (Raw String): {cleaned_prompt[:50]}...")
...
def to_spreadsheet(prompt_text):
    """
    Task to append MULTIPLE generated prompts to the Google Spreadsheet.
    
    This function validates the input list and securely writes the data 
    as new rows in the specified worksheet using batch insertion.
    """
    try:
        ...
        # 2. Data Validation
        # Ensure the prompt_text is a list and contains data.
        if isinstance(prompt_text, list) and len(prompt_text) > 0:
            
            # 3. Write Operation
            # Convert ['A', 'B', 'C'] to [['A'], ['B'], ['C']] for vertical insertion.
            rows_to_append = [[p] for p in prompt_text]
            
            # Append multiple rows at once (Batch Operation).
            ws_process.append_rows(rows_to_append)
            
            print(f"✅ Successfully appended {len(rows_to_append)} prompts to the sheet.")
...
def main_flow():
    ...
    This flow executes the generation task using Gemini, validates the output,
    splits the raw string into a list, and sends the ENTIRE LIST to the 
    spreadsheet task in one go.
    """
    try:
        # 1. Execution Phase
        # Trigger the prompt generation task (Returns a raw String).
        prompt_texts = generate_image_prompts()

        # 2. Validation & Processing Phase
        # Ensure the generated text is valid before processing.
        if prompt_texts and len(prompt_texts.strip()) > 0:
            
            # Split the raw string by Double Newlines.
            prompt_list = prompt_texts.strip().split("\n\n")
            
            all_prompt = []
            original_total = len(prompt_list)
            failed_total = 0

            # Filter out empty prompts and build the valid list.
            for prompt in prompt_list:
                if prompt is not None and len(prompt.strip()) > 0:
                    all_prompt.append(prompt.strip())
                else:
                    failed_total += 1
                    print(f"⚠️ Report: {failed_total}/{original_total} prompts failed to generate/parse.")
                    continue
            
            # 3. Storage Phase
            # Send the COMPLETE LIST of valid prompts to the spreadsheet task.
            # (Assuming to_spreadsheet is configured to handle a list input).
            if all_prompt:
                to_spreadsheet(all_prompt)
                print("✅ Flow completed successfully.")
            else:
                 print("⚠️ Warning: No valid prompts to save.")
